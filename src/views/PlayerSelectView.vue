  ...
  <div class="container mx-auto p-2 text-center md:max-w-md">
    ...
    <div class="player-select min-w-min">
      ...
      <p class="flex-row space-x-2 space-y-2 py-2">
        <div v-for="player in playerStore.players" :key="player" class="inline-block py-2 px-4 bg-blue-900 hover:bg-blue-700 text-white rounded-lg cursor-pointer select-none" @click="remove(player)">
          <span>{{ player }}</span>
        </div>
      </p>
